...
#if ENABLED(DGUS_LCD_UI_SERMOON_V1_CREALITY)
...
const float manual_feedrate_mm_mG[] = { 50*60, 50*60, 4*60, 60 };
...
void RTS_line_to_current(AxisEnum axis) 
{
  if (!planner.is_full())
  {
    planner.buffer_line(current_position, MMM_TO_MMS(manual_feedrate_mm_mG[(int8_t)axis]), active_extruder);
  }
}
...
void RTSSHOW::RTS_Init()
{
  ...
  AxisUnitMode = 1;
    ...
    last_zoffset = bedlevel.z_offset;
    RTS_SndData(offset_negative_001(bedlevel.z_offset) * 100, AUTO_BED_LEVEL_ZOFFSET_VP);
    RTS_SndData((Z_MAX_POS - bedlevel.z_offset) * 100, PRINT_HEITH_VP);
  #else
...
void RTSSHOW::RTS_HandleData()
{
  ...
  switch(Checkkey)
  {
    case MainEnterKey:
      ...
      {
        rtscheck.RTS_SndData((Z_MAX_POS - bedlevel.z_offset) * 100, PRINT_HEITH_VP);
        if(language_change_font != 0)
        {
    ...
    case ZoffsetEnterKey:
        ...
        if(recdat.data[0] >= 32768)
        {
          bedlevel.z_offset = ((float)recdat.data[0] - 65536)/100;
        }
        else
        {
          bedlevel.z_offset = ((float)recdat.data[0])/100;
        }
        if(WITHIN((bedlevel.z_offset), Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX))
        {
          babystep.add_mm(Z_AXIS, bedlevel.z_offset - last_zoffset);
          settings.save();
        }      
        RTS_SndData((Z_MAX_POS - bedlevel.z_offset) * 100, PRINT_HEITH_VP);
      #else
    ...
    case BedLevelKey:
      ...
      else if(recdat.data[0] == 3)
      {
          ...
          RTS_SndData((Z_MAX_POS - probe_offset.z) * 100, PRINT_HEITH_VP);
        #elif ENABLED(MESH_BED_LEVELING)
          if (WITHIN((bedlevel.z_offset + 0.05), Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX))
          {
            bedlevel.z_offset = (bedlevel.z_offset + 0.05);
    ...
    case ManualLevelKey:
      if(recdat.data[0] == 1)
      {
        ...
        {
          bedlevel.z_offset = current_position.z;
